...
The pipeline settings are in `0_config.R`: the water year shown on the site, the date its SWE percentile is calculated for, the last day of data, and the thresholds for which sites get percentiles and charts. The site currently shows April 1, 2026 SWE percentiles, with data through September 23, 2026 for the peak SWE and SM50 charts. To update the site for a new year, change these settings and `p0_fetch_date`, then run `tar_make()` again.
